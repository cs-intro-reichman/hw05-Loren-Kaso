 ...
 */
 ...
 public class GameOfLife {
	...
	public static int[][] evolve(int[][] board) {
		...
		int[][] eveBoard = new int[row][col];
		for (int i = 1; i < row - 1; i++) {
			for (int j = 1; j < col - 1; j++) {
				eveBoard[i][j] = cellValue(board, i, j);
			}
		}
		return eveBoard;
	}
	...
	public static int count(int[][] board, int i, int j) {
		int count = 0;
		for (int k = 0; k < 3; k++) {
			if(board[i-1][j - 1 + k] == 1){     //up row neighbors
				count++;
			}
		}
		for (int l = 0; l < 3; l++) {
			if(board[i+1][j - 1 + l] == 1){		//down row neighbors
				count++;
			}
		}
		if(board[i][j-1] == 1){			//left and right neighbors
			count++;
		}
		if(board[i][j+1] == 1){
			count++;
